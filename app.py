...
<body>
  ...
  <aside>
    ...
    <div class="sidebar-nav">
      ...
      <div class="nav-item" onclick="openImageGeneratorModal()" style="display:flex;align-items:center;gap:8px;margin-bottom:4px;color:#10b981;font-weight:600;">
        <span>🎨</span>
        <span>AI Image Studio</span>
        <span style="margin-left:auto;font-size:10px;background:rgba(16,185,129,0.15);padding:2px 6px;border-radius:9999px;">2 Daily</span>
      </div>
    ...
    <div class="bottom-container">
      ...
      <div class="input-box-wrapper">
        ...
        <div class="input-row">
          ...
          <button type="button" onclick="openImageGeneratorModal()" class="paperclip-btn" title="🎨 Open AI Image Studio (2 free daily)" style="background:rgba(16,185,129,0.15);border:1px solid rgba(16,185,129,0.4);color:#10b981;">
            🎨
          </button>

          <textarea id="user-input" rows="1" placeholder="Ask ChatGPT (GPT-4o), attach a file, or click 🎨..." oninput="this.style.height='auto';this.style.height=(this.scrollHeight)+'px';" onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();submitPrompt();}" class="prompt-textarea"></textarea>
  ...
  <!-- Modal 5: Quick AI Image Studio Modal -->
  <div id="image-generator-modal" class="modal-backdrop" onclick="if(event.target===this) closeImageGeneratorModal()">
    <div class="modal-card" style="max-width:540px;">
      <div class="modal-header">
        <div class="modal-title" style="display:flex;align-items:center;gap:8px;">
          <span>🎨</span>
          <span>Quick AI Image Studio</span>
        </div>
        <button type="button" onclick="closeImageGeneratorModal()" class="modal-close-btn">✕</button>
      </div>

      <div id="img-modal-quota-badge" style="background:rgba(16,185,129,0.12);border:1px solid rgba(16,185,129,0.3);color:#10b981;padding:8px 14px;border-radius:12px;font-size:12px;font-weight:600;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between;">
        <span>✨ 2 Free Daily Images</span>
        <span id="img-quota-remaining-text">2/2 remaining today</span>
      </div>

      <div class="form-group">
        <label class="form-label">Image Prompt / Description</label>
        <textarea id="img-modal-prompt-input" rows="3" class="form-input" style="resize:none;font-family:inherit;line-height:1.5;" placeholder="Describe the image you want to generate (e.g. A futuristic glowing cybernetic city at night)..."></textarea>
      </div>

      <div class="form-group">
        <label class="form-label">Style Presets (Click to add style)</label>
        <div style="display:grid;grid-template-columns:repeat(3, 1fr);gap:8px;">
          <button type="button" onclick="appendImageStyle('photorealistic 8k detail, cinematic lighting')" class="hero-card" style="padding:8px 10px;font-size:11px;text-align:center;justify-content:center;">📸 Photorealistic</button>
          <button type="button" onclick="appendImageStyle('cyberpunk neon aesthetics, octane render 8k')" class="hero-card" style="padding:8px 10px;font-size:11px;text-align:center;justify-content:center;">🎨 Cyberpunk</button>
          <button type="button" onclick="appendImageStyle('anime studio ghibli style, vibrant colors')" class="hero-card" style="padding:8px 10px;font-size:11px;text-align:center;justify-content:center;">✏️ Anime</button>
          <button type="button" onclick="appendImageStyle('unreal engine 5 3d render, hyperdetailed')" class="hero-card" style="padding:8px 10px;font-size:11px;text-align:center;justify-content:center;">🖼️ 3D Render</button>
          <button type="button" onclick="appendImageStyle('epic fantasy digital painting, masterpiece')" class="hero-card" style="padding:8px 10px;font-size:11px;text-align:center;justify-content:center;">🌌 Epic Fantasy</button>
          <button type="button" onclick="appendImageStyle('minimalist vector art, clean aesthetic')" class="hero-card" style="padding:8px 10px;font-size:11px;text-align:center;justify-content:center;">🖌️ Minimal Vector</button>
        </div>
      </div>

      <button type="button" id="img-modal-submit-btn" onclick="submitImageFromModal()" class="google-auth-btn" style="background:#10b981;color:#000000;border:none;margin-top:12px;font-weight:700;font-size:13.5px;padding:12px;">
  ...
  <script>
    ...
    function openImageGeneratorModal() {
      ...
      if (quotaText) {
        ...
        quotaText.innerText = `${q.remaining}/${MAX_DAILY_IMAGES} remaining today`;
      }
    ...
    function appendImageStyle(styleString) {
      const input = document.getElementById('img-modal-prompt-input');
      if (input) {
        const current = input.value.trim();
        if (current) {
          input.value = current + ', ' + styleString;
        } else {
          input.value = styleString;
        }
      }
    }

    function submitImageFromModal() {
      const input = document.getElementById('img-modal-prompt-input');
      if (!input) return;
      ...
      if (mainInput) {
        mainInput.value = '/image ' + promptVal;
      }
      submitPrompt();
    }
    ...
    // Initialize Real Firebase Config (Spark Plan - Project omnira-ai-39c47)
